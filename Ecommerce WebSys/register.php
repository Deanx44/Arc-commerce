<?php 
$page_title = "Register - Arcteryx Shop";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate Password
    if (!preg_match("/^(?=.*[A-Z])(?=.*\d)[A-Za-z\d@$!%*#?&_]{8,}$/", $password)) {
        $errorMessages[] = "Password must be at least 8 characters, with 1 uppercas and numbers.";
    }
    ...
    $(document).ready(function() {
        ...
        function validateField(field) {
            ...
            if (fieldId === 'first_name' || fieldId === 'last_name') {
                ...
                } else if (!/^(?=.*[A-Z])(?=.*\d)[A-Za-z\d@$!%*#?&_]{8,}$/.test(fieldValue)) {
                    error = 'Password must be at least 8 characters, with 1 uppercase and numbers';
                }
