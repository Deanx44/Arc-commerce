...
<div class="d-flex flex-column flex-lg-row">
    ...
    <div class="flex-grow-1">
        ...
        <?php if ($action === 'list'): ?>
            <p><a href="products.php?action=add" class="btn btn-primary mb-3"><i class="bi bi-plus-circle"></i> Add New Product</a></p>
        ...
        <?php elseif ($action === 'add' || ($action === 'edit' && $current_product)): ?>
            <h2><?php echo $action === 'add' ? 'Add New Product' : 'Edit Product: ' . htmlspecialchars($current_product['name']); ?></h2>
            <form action="products.php<?php echo $action === 'edit' ? '?action=edit&id=' . $product_id : '?action=add'; ?>" method="POST" enctype="multipart/form-data">
                ...
                <hr>
                <button type="submit" name="save_product" class="btn btn-success"><i class="bi bi-save"></i> Save Product</button>
